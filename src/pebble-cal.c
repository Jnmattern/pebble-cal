...
void updateMonth(Layer *layer, GContext *ctx) {
    ...
    int i, x, s, numWeeks, dy, firstday, numDays, l=0, c=0;
    Date first, d;
    ...
    graphics_fill_rect(ctx, GRect(DX+DW, dy, DW*7+1, DH), 4, GCornersTop);
	...
	graphics_draw_round_rect(ctx, GRect(DX+DH, dy+DH, DW*7, numWeeks*DH), 0);
    ...
    for (i=1; i<=(numWeeks+1); i++) {
        graphics_draw_line(ctx, GPoint(DX+DW,dy+DH*i), GPoint(DX+DW*8,dy+DH*i));
    }
	
	fontNorm = fonts_get_system_font(FONT_KEY_GOTHIC_14);
	fontBold = fonts_get_system_font(FONT_KEY_GOTHIC_14_BOLD);
	f = fontNorm;
...
#endif
    for (i=s; i<s+7; i++) {
		graphics_text_draw(ctx, weekDays[i%7], f, GRect(DX+DW*(i+1-s), dy, DW+2, DH), GTextOverflowModeTrailingEllipsis, GTextAlignmentCenter, NULL);
	}
	...
	for (i=1; i<=numDays; i++) {
		...
		rect = GRect(DX+DW*(c+1), dy+DH*(l+1)-1, DW, DH);
		
		if (today.day == i && today.month == displayedMonth && today.year == displayedYear) {
			graphics_fill_rect(ctx, GRect(rect.origin.x, rect.origin.y, rect.size.w, rect.size.h+1), 0, GCornerNone);
			graphics_context_set_text_color(ctx, GColorWhite);
...
void handle_init(AppContextRef ctx) {
	...
	text_layer_init(&monthNameLayer, GRect(0, 0, 144, 20));
	...
	layer_init(&monthLayer, GRect(0, 20, 144, 132));
